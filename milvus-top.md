  ...
  - `key` 对应 `S3` 的数据文件
    ...
    - `value` 内容包含
      ...
      - `replicas`： 插入该文件及拥有该文件 `replica` 的 `Milvus` 节点序号,
        ...
        - 假设该文件由节点`2`插入，并且节点 `0` 和 节点 `5` 都拥有该文件的 `replica`，但是节点`0` 正在复制这个文件，并未完成，而节点`5` 已经完成复制，此时这个值为 `[2,-0,5]`
      - 如果为标量文件，还需记录以下内容
        - 对应的列名
        - 当前标量文件对应的向量文件名
...
- 根据 `meta` 信息，可能出现某个 `replica` 正在复制的过程中，假设 `F0` 正在往 `N1` 复制的过程中，此时对 `F0` 的查询在节点 `N2` 上执行
- `meta` 信息显示 `F0` 正在往 `N1` 复制，但是 `N2` 已经宕机，如何处理?
  - 向 `N1` 发送查询指令，观察 `N1` 是否正在执行复制任务
  - 如果 `N1` 正在执行复制任务任务，切数据源为 `S3`，则 `N0` 等待 `N1` 复制完成后在执行
  - 如果 `N1` 的任务列表内没有复制任务，则 `N0` 向 `N1` 发送复制任务，指示 `N1` 从 `S3` 复制 `F0` ， 等待复制完成后在执行查询 
...
## 插入流程
- `client` 向 `Milvus` 节点 `N0` 插入数据，生成文件 `F0`
- `N0` 将文件 `F0` 写入 `S3`
- `N0` 向其它节点 `N1 ~ N5` 发送查询请求，询问其负载状态
- 根据 `N1 ~ N5` 的负载状态，及配置文件中 `num_replicas`，`N0` 决定将 `F0` 分别复制到 `N2` 和 `N5`
- 向 `etcd` 中插入 
...
- 如果 `N0` 已经向 `etcd` 插入 `meta`，但是还未向 `N2` 和 `N5` 发送复制指令，此时 `N0` 宕机了，如何处理？ 
